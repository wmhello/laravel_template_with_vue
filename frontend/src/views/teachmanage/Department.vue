<template>
  <div id="department">
       <el-form id="toolbar" :inline="true" :model="searchForm" class="demo-form-inline">
          ...
          <el-form-item label="组长类型">
             <el-select v-model=" searchForm.leader" clearable placeholder="年级">
                 <el-option label="备课组长" value="0"></el-option>
                 <el-option label="教研组长" value="1"></el-option>
             </el-select>
         </el-form-item>
  ...
  <div id="datagrid">
    ...
    <el-table :data="tableData" border style="width: 100%" @select-all="selectChange" @selection-change="selectChange" v-loading="loading">
      ...
      <el-table-column label="组长类型" width="100">
        <template slot-scope="scope">
          <el-tag
          :type="scope.row.leader === 1 ? 'primary' : 'info'"
          close-transition>{{scope.row.leader|getLeader()}}</el-tag>
        </template>
      </el-table-column>
    ...
    <el-pagination
      ...
      :total="total">
        ...
        <!-- 编辑列表 -->
  <el-dialog title="教研组长信息" center :visible.sync="editDialogFormVisible" :close-on-click-modal="false">
      <el-form :model="form" label-width="100px" label-position="top">
        ...
        <el-row class="normal-row">
          ...
          <el-col :span="10">
        <el-form-item label="组长类型">
           <el-radio v-model="form.leader" :label="0" border>备课组长</el-radio>
           <el-radio v-model="form.leader" :label="1" border>教研组长</el-radio>
        </el-form-item>
          </el-col>
        </el-row>

        <el-row class="last-row">
        <el-col :span="20" class="first-column" :offset="2">
...
import { getSession, getTeacher, getDefaultSession, getTeach} from "@/api/other";
...
export default {
  ...
  data() {
    return {
      ...
      pageSize: 10,
      session_id: null,
      templateFile: config.site + '/xls/department.xls',
      downloadFile: config.site + '/xls/教研组长管理.xls',
      multiSelect: []
    };
  },
  methods: {
