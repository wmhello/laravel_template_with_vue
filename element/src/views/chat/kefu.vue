    ...
    <el-row>
      <el-col :span="16">
        ...
        <div class="chat-box" v-if="kefu !== name || users.length === 0">
          <div v-for="item in messages" :key="item.timezone"><chat-item :item="item" :name="name"></chat-item></div>
        </div>
        <!-- 客服页面 -->
        <el-tabs tab-position="left" style="margin-top: 20px;" v-if="kefu === name && users.length >= 1" v-model="currentUser">
          <el-tab-pane :label="item" :name="item" v-for="item in users" :key="item" v-if="kefu !== item">
            <div class="chat-box">
              <div v-for="msg in kefuMsg[item]" :key="item.timezone"><chat-item :item="msg" :name="name"></chat-item></div>
            </div>
          </el-tab-pane>
        </el-tabs>
        <div class="chat-box warnTips" v-if="users.length === 1 && users[0] === kefu"><h1>暂时没有用户接入客服，请耐心等待</h1></div>
      </el-col>
        ...
        <el-card class="box-card">
          ...
          <div v-if="name === kefu" style="margin-bottom:20px">
            <span v-for="item in users" :key="item">
              <!-- 当前登录的是客服，则不显示客服本身 -->
              <el-radio @change="changeCurrentUser" style="margin-right: 20px;" v-if="item !== kefu" v-model="currentUser" :label="item" border>{{ item }}</el-radio>
            </span>
          </div>
          <el-form class="chat-msg" :inline="true" :model="formInline" @submit.native.prevent="test">
            <el-form-item label="消息"><el-input v-model="formInline.msg" @keyup.enter.native.prevent="send"></el-input></el-form-item>
            <el-form-item><el-button type="success" plain @click.native.prevent="send">发送</el-button></el-form-item>
          </el-form>
        </el-card>
        <el-card class="box-card">
...
<script>
import { getCustomer, sendKefu } from '@/api/chat';
